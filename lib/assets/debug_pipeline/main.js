export async function init(ctx, payload) {
  await importJS("https://cdn.jsdelivr.net/npm/vue@3.2.37/dist/vue.global.prod.js");
  await importJS("https://cdn.jsdelivr.net/npm/vue-dndrop@1.2.13/dist/vue-dndrop.min.js");
  ...
  const app = Vue.createApp({
    ...
    <div class="app">
      <div class="navigation">
        <div class="headline">
          <span>{{ dbgLocationInfo }}</span>
          <span v-if="callCount > 1">
            (showing first out of {{callCount}} calls)
          </span>
        </div>
        <div class="actions">
          <div class="copy-source">
            ...
            <button id="help-toggle" @click="copySource" class="icon-button">
    ...
    data() {
      return {
        dbgLocationInfo: payload.dbg_location_info,
        items: payload.items,
    ...
    computed: {
      abortedIds() {
        if (this.erroredId === null) {
          ...
          const erroredIdx = this.items.findIndex((item) => item.id === this.erroredId);
    ...
    methods: {
      ...
      handleEnabledChange(item, event) {
        ctx.pushEvent("update_enabled", { id: item.id, enabled: event.target.checked });
      },
  ...
  ctx.handleEvent("enabled_updated", ({ id, enabled, selected_id, changed }) => {
  ...
  ctx.handleEvent("source_reply", ({ source }) => {
