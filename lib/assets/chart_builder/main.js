export function init(ctx, info) {
  ...
  function setAvailableData(options) {
    ...
    if (!availableData) {
      createOptions(selectElem, [selectedData], "unavailable");
      selectElem.value = selectedData;
      disableAll();
    ...
    if (availableData) {
      for (const elem of channelFields) {
        ...
        const inner = elem === "color" ? "<option value=''></option>" : "";
  ...
  function setAxisOptions(options) {
    enableAll();
    const unavailableData = ctx.root.querySelector(".unavailable");
    if (unavailableData) { unavailableData.remove() }
    for (const elem of channelFields) {
      const selectElem = ctx.root.querySelector(`[name="${elem}"]`);
      const inner = elem === "color" ? "<option value=''></option>" : "";
      selectElem.innerHTML = inner;
