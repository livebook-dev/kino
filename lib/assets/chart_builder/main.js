export function init(ctx, info) {
  ...
  setDataOptions(info.options);
  setValues(info.fields);

  const hasType = ["x_axis", "y_axis", "color"];
  const types = ["x_axis_type", "y_axis_type", "color_type"];
  ...
  function handleFieldChange(event) {
    ...
    if (hasType.includes(name)) { maybeReset({name, value}) }
  ...
  function missingData() {
    for (const name of hasType.concat(types)) {
  ...
  All datasets declared above a chart builder will be available on it.`;
    }
  }
}
