export function init(ctx, payload) {
        ...
        <div class="row">
          ...
          <div class="box">
            <div class="field">
              ...
              <input class="input input--md" name="chart_title" />
            </div>
          </div>
        </div>
        <div class="row">
  ...
  function maybeResetFieldOptions(event) {
    const name = event.target.name;
    const selectElemType = ctx.root.querySelector(`[name="${name}_type"]`);
    const selectElemAggregate = ctx.root.querySelector(`[name="${name}_aggregate"]`);
    const { value } = event.target;
    if (value && value !== "__count__") {
      selectElemType.disabled = false;
      selectElemAggregate.disabled = false;
    } else {
      resetField(selectElemType);
      resetField(selectElemAggregate);
    }
  }
  ...
  function createOptions(selectElem, options, elemClass = null) {
    ...
    if (elemClass !== "unavailable-option") {appendCountOption(selectElem);}
  }

  function appendCountOption(selectElem) {
    const element = document.createElement("option");
    element.value = "__count__";
    element.innerText = "COUNT(*)";
    element.classList.add("special-option")
    selectElem.append(element);
  }
  ...
  function channelFieldsListener() {
    for (const elem of channelFields) {
      const selectFieldElem = ctx.root.querySelector(`[name="${elem}"]`);
      selectFieldElem.addEventListener("change", maybeResetFieldOptions);
    }
  }
